...
pub fn spine_surfaces() -> Vec<SpineSurface> {
    vec![
        ...
        SpineSurface {
            ...
            gap: "Chat, memory-write proxies, MCP invoke, screen-capture, browser-capture, plugin management mutations, outbound email, and agent-message send paths can attach to a task; remaining direct utility invocations still need forced task or explicit ad-hoc run binding.",
        },
        SpineSurface {
            id: "runs",
            label: "Runs / Attempts",
        ...
        SpineSurface {
            ...
            gap: "Durable activity JSONL now captures workspace, intervention, approval, policy-denial, cost-observation, memory-write, MCP, screen, browser-capture, plugin management, outbound email, and agent-message events; model and dispatch producers still need the same ledger path.",
        },
        SpineSurface {
            id: "memory",
