...
pub struct QdrantConfig {
    ...
    #[serde(default)]
    pub url: String,
    /// Collection name. Defaults to `relix_memory`.
...
impl QdrantClient {
    ...
    /// issued immediately after sees the new vectors.
    ...
    pub async fn delete_by_filter(&self, filter: serde_json::Value) -> Result<u64, QdrantError> {
        ...
        let resp = self.auth(self.http.post(&url)).json(&body).send().await?;
        // Qdrant returns `{"result": {"operation_id": N, "status":"completed"}}`
        // for delete. There's no count in the standard response;
        // we treat a 2xx as success and return 0 when the body
        // doesn't carry one. Callers use this primarily as a
        // confirmation, not a tally.
...
mod tests {
    ...
    async fn upsert_sends_put_with_points_array() {
        ...
        assert_eq!(cap.len(), 1);
        assert_eq!(cap[0].method, "PUT");
        assert!(cap[0].path.starts_with("/collections/test_coll/points"));
        let pts = &cap[0].body["points"];
        assert!(pts.is_array());
    ...
    async fn search_round_trips_request_and_response() {
        ...
        assert_eq!(cap[0].path, "/collections/test_coll/points/search");
        assert_eq!(cap[0].body["limit"], 10);
        assert!((cap[0].body["score_threshold"].as_f64().unwrap() - 0.75).abs() < 1e-5);
        assert!(cap[0].body["filter"].is_object());
    }
