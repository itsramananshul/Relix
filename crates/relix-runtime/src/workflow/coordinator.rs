...
pub fn register(
    ...
    register_run(bridge, store.clone(), chronicle.clone(), dispatcher_cell);
    register_list(bridge, store.clone());
    register_status(bridge, chronicle);
    register_validate(bridge, store, known_peers);
}
...
async fn handle_run(
    ...
    match serde_json::to_vec(&record) {
        Ok(body) => match result.status {
            ExecutionStatus::Success => HandlerOutcome::Ok(body),
            ExecutionStatus::Failed => HandlerOutcome::Ok(body),
        },
