...
**Status:** SOL `RemoteCall` opcode + dispatcher + flow-log tracing landed in M6/S4. `flows/ping.sol` is the M6 single-peer demo; `flows/chained_health.sol` is the M6/S7 multi-peer demo. M7 ships the first two real nodes: memory (SQLite + FTS5) and AI (provider-abstracted: `mock` default, `anthropic` available behind env-var/file API key). `flows/chat.sol` is the conversational state machine — persist user → read recent → AI call → persist assistant; 10-event happy-path flow log; both responders audit independently. The same SOL flow runs unchanged across both providers.
