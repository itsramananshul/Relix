...
**Last updated:** May 29, 2026 — §7.18 Research-Backed Identity flipped to `[DONE]` (commits 5c18f41 + 2bde84d + 34465a5 + 061634a; GAP 17 closed). Operators paste a Tavily / Brave / Perplexity key via scripts/setup.{sh,ps1}; the controller auto-selects from env at startup, the coordinator wires the five-stage ResearchPipeline behind the identity.research cap + POST /v1/identity/research + relix identity research --subject. Prior audit pass (May 27): Dependency Auto-Install shipped (cd9ea63); 14 items flipped to `[SKIPPED]` with the honest reason each requires (external infrastructure, external API access, multi-week frontend work, or a dedicated session for cross-cutting cryptographic / policy review). Memory Security, Memory Inspector, and Memory Consolidation Strategy flipped to `[DONE]` against their already-shipped commits. §7.23 Perception Tools flipped to `[PARTIAL]` (Browser + Audio shipped, four other tools deferred).
...
>
> **NOT DONE — provider-cost-spike + ask-human-rate drift alerts.** The original Feature 2 listed two "quality drift" alert categories that ride on top of metric histories that don't exist today: (a) "OpenRouter costs jumped 40% today vs yesterday average" requires per-provider cost time-series we don't bucket on disk yet, and (b) "ask_human called 40% of sessions today vs 5% baseline" requires per-method invocation-rate baselines per session, which the metrics store doesn't index. The shipped 7.28 dashboard surfaces every alert kind the engine knows how to fire (`ErrorRate`, `P95Latency`, `CostPerHour`, `ZeroSuccess`, `LowConfidence`, `BudgetExceeded`), and the architecture is ready to receive new kinds — the gap is the rolling-baseline computation, not the delivery path. Deferred to a follow-up that lands the provider-cost time-series + ask-human-rate counter alongside their alert evaluators.
>
