...
### 7.5 Multi-Agent Workflow Foundation `[DONE — commits 8aecd98 + 03a02b1 + 9c29b5d + 4d4832a]`
...
- **Bridge endpoints (commit 9c29b5d)**: `POST /v1/workflows/run` (SSE-wrapped final result when `stream: true`), `GET /v1/workflows`, `GET /v1/workflows/:name/status/:execution_id` (404 on miss), `POST /v1/workflows/validate` (400 + clear error on parse / validation failure). Error mapping: INVALID_ARGS → 400, "not found" → 404, "not ready" / "not wired" → 503, other responder errors → 502. End-to-end mini-mesh integration test boots a fake coordinator peer with canned `workflow.*` responses, builds a real `MeshClient` via `discover_and_pin`, and asserts the three required scenarios.
- **CLI (commit 4d4832a)**: `relix workflow list` (catalog table), `relix workflow run <name> --input <text>` (executes + renders result + per-step trace table), `relix workflow validate <file>` (exits 2 on failure for CI gating), `relix workflow trace <execution-id>` (renders a past run's trace). Every subcommand accepts `--bridge <url>` and `--raw` for scripting.

**Skipped sub-items (called out individually per the roadmap discipline):**

- **Dashboard visual workflow builder** — drag-and-drop graph editor with bidirectional flow source sync is multi-week frontend work and is intentionally deferred to a dedicated dashboard sprint. The engine + capabilities + bridge endpoints + CLI shipped in this session are the foundation the dashboard will sit on; the dashboard adds presentation, not capability.
- **Per-step SSE streaming on `POST /v1/workflows/run`** — `stream: true` returns the final execution record as a single `text/event-stream` `result` event. True per-step streaming requires a streaming variant of the coordinator's `workflow.run` capability (the executor would emit step events to a channel; the bridge would pipe each one as its own SSE frame). The event-name discipline of the current implementation (`event: result`) is forward-compatible so dashboard clients pick up streaming once the streaming variant ships.
