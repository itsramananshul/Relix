...
### 7.29 Reasoning and Decision Engine — Smart Routing, Confidence, Belief Tracking, and Judge Model `[DONE — RELIX-7.29 rebuild commits 0fef9cc + c9d5327 + 3d8862d + bf005dd + (PART 5 wire-up)]`
...
> - **PART 5 — Wire everything + `relix reasoning status`** (`(this commit)`): `reasoning_status::ReasoningStatus` snapshots every component's live state in one JSON body. `reasoning.status` cap + `GET /v1/reasoning/status` + `relix reasoning status` CLI prints a per-component summary with the live counters from PARTS 1-4. 3 new unit tests.
>
