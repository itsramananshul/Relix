...
>
> **NOT DONE — known limitations carried into future work.** Three production-quality limitations are documented here so future operators don't re-discover them as bugs:
> - **No mid-execution workflow cancellation.** The Stage-5 verification harness records failures and overrides the final WorkflowResult status to `Failed`, but the workflow engine itself has no cancel primitive — subsequent steps still run if their source edges fire. The operator's observable outcome is identical to a halt (run reported as failed, log carries the cause), but the spec's literal "halt the workflow" wording is approximated by status override rather than implemented as a true cancel signal. A future workflow-engine extension that threads a `tokio::sync::CancellationToken` through `execute_with_events` would close this gap; the harness would then propagate a cancel on the first critical failure.
> - **No streaming verification progress in `relix build`.** The CLI prints the full verification log AFTER `planning.create_plan` returns. A streaming SSE bridge endpoint that emits per-step verification events as they're recorded would give operators true live progress; it's a separate scope from this milestone because exposing the workflow event stream over SSE for planning needs a new bridge route and a new streaming-cap shape on the coordinator.
> - **No formal export targets for the hardened PlanSpec yet.** The serialised PlanSpec is stable + signed and external tools can consume it, but there's no built-in importer for common trackers (Linear / GitHub Issues / Jira) — operators that want round-trip export against those systems still write their own glue. This is intentional: pinning to a specific external tool inside the runtime would couple the planner to one tracker's schema, and we'd rather operators tooling-up around our stable JSON shape than vice versa.
>
> The §7.24 design is closed as a shipping milestone. Future planning work belongs in new roadmap sections — the four limitations above plus any new feature that isn't already covered by the orchestrator / critic / conflict-resolver / approval-gate / verification-harness primitives.
