...
### 7.29 Reasoning and Decision Engine — Smart Routing, Confidence, Belief Tracking, and Judge Model `[DONE — rebuild 0fef9cc + c9d5327 + 3d8862d + bf005dd + b36e3c1; deferred follow-ups closed 2ffc41e + b589c36 + (cleanup commit)]`
...
> - **Pre-rebuild cleanup** ((cleanup commit)) — deleted `crates/relix-runtime/src/nodes/ai/reasoning/` (mod, config, classifier, tier_router, belief, judge, confidence_signals), `nodes/ai/reasoning_caps.rs`, and `nodes/ai/belief_caps.rs`. Removed every `reasoning::*` import + the `[ai.reasoning]` AiConfig field + the `reasoning_config` param on `ai::register`. Stripped the legacy GAP-16 smart-router blocks from `handle_chat` + `handle_chat_stream` (the spec'd `tier_routing::TierRouter` already handles routing). Updated every test site that previously passed `reasoning::TierRouter::default()` etc. — the new signatures take only the spec'd modules.
