...
- **How it works (verified):** spawn `codex exec --json [...] -`, **pipe the prompt over stdin**, `--model`, `-c model_reasoning_effort=…`, fast-mode flags, `resume <session>` for continuation. Parse JSONL back.
